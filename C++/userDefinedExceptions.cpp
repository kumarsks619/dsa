...
int avgArray(int arr[], int n)
{
    if (n == 0)
    {
        throw ArraySizeZero(); // throwing an user-defined data type execution.
    }
    if (n < 0)
    {
        throw ArraySizeNegative(); // throwing an user-defined data type execution.
    }
...
// It is a good practice to throw user-defined data-types as exceptions.

// Watch GFG C++ 25 : for pre-defined exception classes and their inheritance.