...
// this SmartPtr class can handle pointer of Test type ONLY.
class SmartPtr
{
    Test *ptr;
...
public:
    ...
    Test &operator*()
    { // * is overloaded to return the obj of Test class whenever SmartPtr obj is used.
        cout << "SmartPtr: * Operator overloaded..." << endl;
        return *ptr;
    }
    Test *operator->()
    { // -> is overloaded to access the members of Test class.
        cout << "SmartPtr: -> Operator overloaded..." << endl;
        return ptr;
    }
    ~SmartPtr()
    { // this destructor will lead to Test class destructor.
...
int main()
{
    ...
    { // anywhere in the programme, a scope can be created by simply using these curly braces.
