...
void bfs(vector<int> adj[], int src, bool visited[])
{
    ...
    while (!q.empty())
    {
        int v = q.front();
        q.pop();
        cout << v << " "; // single point of process: only here the processing of the Node takes place
        for (int u : adj[v])
        {
            if (!visited[u])
            {
...
// function to visit and process (exactly once) all the Nodes of a graph even if they are NOT connected in BFS manner
void bfsDisconnected(vector<int> adj[], int V)
{
    ...
    for (int i = 0; i < V; i++)
    {
        ...
        {
            bfs(adj, i, visited); // calling the BFS with the current Node (i) as the source
            // after this all the Nodes somehow connected to the current Node will get processed for sure so the bfs() won't be called
...
int main()
{
    ...
    // here the graph is not connected altogether (3 SEPARATE ISLANDS)
...
// Time Complexity of bfsDisconnected() ---> O(V + E)
