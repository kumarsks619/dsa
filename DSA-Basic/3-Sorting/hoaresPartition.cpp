...
int haoresPartition(int arr[], int l, int u, int p)
{
    ...
    while (true)
    { // running the loop till the return call has made
        do
        {
        ...
        if (i >= j)               // checking if the left and right side boundaries haven't crossed each other yet
            return j;             // if crossed then the partition has completed and return the index of the final common boundary
...
int main()
{
    ...
    cout << endl
         << "Boundary Index of the partition: " << boundaryIndex;
