...
int lomutoPartition(int arr[], int l, int u, int p)
{
    ...
    for (int j = l; j < u; j++)
    { // Transversing the array only ONCE
        if (arr[j] < pivot)
        {                         // Take action only when the element is smaller than the pivot element
...
int main()
{
    ...
    cout << endl
         << "Correct Pivot Index: " << correctPivotIndex;
...
// UNSTABLE Algorithm - the order of occurrence of equal elements may change.
// No Auxiliary Array needed. [IN-PLACE]

// Naive Partition Method - Stable but requires a Theta(n) Auxiliary Space