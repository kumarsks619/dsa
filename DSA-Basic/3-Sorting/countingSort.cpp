...
{
    int count[k];               // array to keep the count of occurrences of every element of the MAIn array
    ...
    for (int i = 0; i < n; i++) // transversing the MAIN array
        count[arr[i]]++;        // counting the occurrences of each element

    for (int i = 1; i < k; i++)             // transversing the COUNT array starting form index 1
        count[i] = count[i - 1] + count[i]; // converting the count array to CUMULATIVE COUNT array

    int output[n]; // array to build the output (SORTED) array
    for (int i = n - 1; i >= 0; i--)
    {                                       // transversing the MAIN array backward to keep the STABILITY
        output[count[arr[i]] - 1] = arr[i]; // Building the OUTPUT array:  putting the element of the MAIN array based on the position
        // provided by the CUMULATIVE COUNT array in the OUTPUT array.
        count[arr[i]]--; // decrementing the position value of that particular element that has been already placed
...
// Working principle: keeps the count of occurrences of elements in a separate array (associated with indexes) and then put the elements
...
// STABLE Algorithm: preserves the order of occurrences of equal elements in the sorted array.
