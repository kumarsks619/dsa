...
{
    for (int i = 0; i < n - 1; i++)
    {                     // loop to take care of all the positions
        int minIndex = i; // to keep the index of the smallest(or largest) element
        for (int j = i + 1; j < n; j++)
        { // loop starts after the sorted array to find the smallest(or largest) element index
...
// Selection sort is an "UNSTABLE algorithm" - the order of occurrence of equal values may change.
