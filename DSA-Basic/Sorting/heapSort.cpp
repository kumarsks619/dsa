...
void maxHeapify(int arr[], int n, int i) {
    ...
    if (right < n && arr[right] > arr[largest]) // comparing the NODE with its RIGHT children
        largest = right;
    if (largest != i) {              // checking if the NODE is already greatest among its TWO children
...
void heapSort(int arr[], int n) {
    ...
    for (int i = n-1; i >= 1; i--) {    // reducing the size of the array one by one ([UNSORTED, SORTED])
