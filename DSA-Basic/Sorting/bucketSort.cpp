...
void bucketSort(int arr[], int n, int k)
{
    ...
    maxVal += 1; // incrementing the maxVal by 1 so that bucketIndex will always fall in range [0, k-1]

    // STEP 2: creating buckets and distributing the elements among these buckets.
    vector<int> bucket[k];
    for (int i = 0; i < n; i++)
    {                                            // transversing through the main array
...
// Working principle: re-distribute the given array elements in various ranges (sub-arrays) and then sort those sub-arrays individually and
// finally put them together to form the main sorted array.
// BEST CASE: when the array elemets are uniformly distributed over the range : [Time Complexity: O(n)] - when used insertion sort (small sub-arrays)
