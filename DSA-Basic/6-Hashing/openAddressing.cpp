...
struct myHash
{
    ...
    bool search(int key)
    {
        ...
        int i = h;
        while (arr[i] != -1)
        {                      // until an empty slot is NOT found  [search operation doesn't stop at a deleted location (flag = -2)]
    ...
    bool insert(int key)
    {
        ...
        {                 // else inserting procedure because either an empty slot or a deleted slot is found
    ...
    bool erase(int key)
    {
        ...
        int i = h;
        while (arr[i] != -1)
        { // until an empty slot is NOT found
            if (arr[i] == key)
            {                // if the key is found
