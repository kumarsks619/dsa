...
int leftMostNonRepeat(string str) {
    int firstIndex[256];                // array to keep the index of ONLY first occurence of each character in the string
    for (int i = 0; i < 256; i++)
        firstIndex[i] = -1;             // initializing the whole array with -1

    for (int i = 0; i < str.length(); i++)
        if (firstIndex[str[i]] == -1)           // checking for first occurence
            firstIndex[str[i]] = i;             // if first occurence then store its index in the array
        else    
