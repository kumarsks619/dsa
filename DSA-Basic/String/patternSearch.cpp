...
void searchPattern(string txt, string pat) {
    ...
    for (int i = 0; i <= n-m; i++) {                            // transversing the window to cover the whole Text string
        for (int j = 0; j < m; j++) {                           // transversing the WINDOW to match characters
            ...
            if (j == m-1)                                       // if the WINDOW is transversed completely
                cout << "Occured at index: " << i << endl;      // print the first index of the WINDOW
        }
    }
}
...
// occurences of the PATTERN string in the TEXT string.
