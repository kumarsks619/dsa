...
void printNthNodeFromEnd(Node *head, int n) {
    ...
    cout << endl << curr->data;     // now the curr ptr will be at Nth position from end
}
...
void printNthNodeFromEndPtrs(Node *head, int n) {
    ...
    Node *first = head;
    for (int i = 0; i< n; i++) {        // STEP 1: moving the first pointer by n positions 
        first = first->next;
    ...
    while (first != NULL) {       // when the first ptr reaches end (NULL) then the second ptr will be at our desired position
        second = second->next;
        first = first->next;
    }
    cout << endl << second->data;       // now the second ptr will be at Nth position from end
}
